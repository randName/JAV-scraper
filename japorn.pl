...
my $JAP = "http://www.aventertainments.com";
...
sub write_to_file
{
	my $fn = shift; open ( JAP, ">$fn" ) || die "Can't open: $!\n";
	print JAP JSON->new->utf8->encode( @_ ); close( JAP );
}

sub read_from_file
{
	my $fn = shift; 
	my $_text = do { open( JAP, "<:encoding(UTF-8)", $fn) or die("Error"); local $/; <JAP> };
